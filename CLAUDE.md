...
- TypeScript build configured (`npm run build`)
- No test framework configured
- No linting or formatting tools configured
- No CI/CD configuration
- `package.json` entry point is `index.js` but source uses `src/index.ts`
- PostgreSQL 数据库存储已实现：`InitDatabase`、`CreateNewSessionInDatabase`、`GetAllSessionsFromDatabase`、`ReadSessionDataFromDatabase`，使用 database 时需在 `InputOptions` / `CombineContextOptions` 中传入 `databaseConfig`
...
在开始工作之前应该先看一下最新的目录结构是怎样的，因为我们可能每一次发送prompt之前都是会更新一下名称或者位置的等，要按照最新的目录结构来完成任务
